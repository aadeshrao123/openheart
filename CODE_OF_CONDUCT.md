...
This project uses the Contributor Covenant, version 3.0, reproduced below. The
only changes are ASCII apostrophes in place of curly ones, rewrapped lines, and
the added note under Reporting an Issue.
...
> **This reporting route is a placeholder and has not been filled in yet.**
> A maintainer must replace the bracketed note above with a real, monitored,
> private contact before this document can be relied on. Do not substitute the
> security advisory route: that is for vulnerabilities only, and it is
